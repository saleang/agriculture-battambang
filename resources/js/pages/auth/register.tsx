...
export default function Register() {
  ...
  // ---- FIX 1: use a ref so the hidden submit can be triggered ----
  ...
  useEffect(() => {
    ...
    if (!validateEmail(data.email)) { setEmailStatus('invalid'); return; }
  ...
  useEffect(() => {
    ...
    if (!validatePhone(data.phone)) { setPhoneStatus('invalid'); return; }
  ...
  // ---- FIX 2: derive last step correctly ----
  // Customers: step 1 → step 3 (skip 2). Sellers: step 1 → 2 → 3.
  const lastStep = isSeller ? 3 : 2; // visual step count
  const isOnLastStep = isSeller ? currentStep === 3 : currentStep === 3;
  ...
  const isStep1Valid = () => {
    const basicValid =
      ...
      emailStatus === 'valid' &&
      phoneStatus === 'valid';
    if (isSeller) return basicValid && data.farm_name.trim().length > 0;
    return basicValid;
  };
  ...
  const isStep3Valid = () => {
    ...
    return passwordValid && data.password === data.password_confirmation;
  };
  ...
  const handlePrev = () => {
    if (currentStep === 3) {
      setCurrentStep(isSeller ? 2 : 1);
    } else if (currentStep === 2) {
      setCurrentStep(1);
    }
  };

  // ---- FIX 3: submit lives on the <form>; the button just calls formRef.current.requestSubmit() ----
  const submit: FormEventHandler<HTMLFormElement> = (e) => {
    e.preventDefault();
    if (!isStep1Valid() || !isStep3Valid() || (isSeller && !isStep2Valid())) return;

    post('/register', {
      onError: (errs) => console.error('❌ Registration error:', errs),
      onSuccess: (response) => console.log('✅ Registration success:', response),
    });
  };
  ...
  // Visual step number for display
  const displayStep = !isSeller && currentStep === 3 ? 2 : currentStep;
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-emerald-50 p-2 sm:p-4">
        <div
          className="w-full max-w-6xl bg-white rounded-xl lg:rounded-2xl shadow-2xl overflow-hidden flex flex-col lg:flex-row"
        ...
        >
          ...
          <div className="w-full lg:w-2/3 flex flex-col overflow-hidden">
            <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full overflow-hidden">
              ...
              <div className="flex-shrink-0 mb-4 lg:mb-6">
                ...
                <div className="mb-3 lg:mb-4">
                  <div className="flex items-center justify-between relative max-w-md mx-auto">
                    ...
                    {/* Always show step 1 */}
                    {[1, 2, 3].map((step) => {
                      if (!isSeller && step === 2) return null; // hide location step for customers
                      const visualNum = !isSeller && step === 3 ? 2 : step;
                      const isActive = currentStep >= step;
                      return (
                        <div key={step} className="relative z-10 flex flex-col items-center flex-1">
                          <div
                            className={`w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center border-2 transition-all ${
                              isActive ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white border-gray-300 text-gray-400'
                            }`}
                          >
                            <span className="text-[10px] sm:text-xs lg:text-sm font-medium">{visualNum}</span>
                          </div>
              ...
              {/* ---- FIX 4: entire step content is INSIDE the <form> ---- */}
              <form ref={formRef} onSubmit={submit} className="flex-1 overflow-hidden flex flex-col">
                      ...
                      <div className="space-y-4 lg:space-y-5 animate-fadeIn">
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-2.5 sm:gap-3 lg:gap-4">
                            <button
                              ...
                              className={`p-3 sm:p-4 lg:p-5 border-2 rounded-lg lg:rounded-xl text-center transition-all ${
                                data.role === 'customer' ? 'border-emerald-600 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'
                              }`}
                            >
                            ...
                            <button
                              ...
                              className={`p-3 sm:p-4 lg:p-5 border-2 rounded-lg lg:rounded-xl text-center transition-all ${
                                data.role === 'seller' ? 'border-emerald-600 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'
                              }`}
                            >
                              <div className="flex flex-col items-center gap-1.5 lg:gap-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 sm:gap-3 lg:gap-4">
                          ...
                          <div>
                            ...
                            {emailStatus === 'taken' && <p className="text-red-600 text-[10px] sm:text-xs mt-0.5 lg:mt-1">អ៊ីមែលនេះត្រូវបានប្រើប្រាស់រួចហើយ</p>}
                            {emailStatus === 'invalid' && data.email.length > 0 && <p className="text-red-600 text-[10px] sm:text-xs mt-0.5 lg:mt-1">អ៊ីមែលមិនត្រឹមត្រូវ</p>}
                          ...
                          <div>
                            ...
                            {phoneStatus === 'taken' && <p className="text-red-600 text-[10px] sm:text-xs mt-0.5 lg:mt-1">លេខទូរស័ព្ទនេះត្រូវបានប្រើប្រាស់រួចហើយ</p>}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
                          ...
                          <div>
                            ...
                            <div className="mt-2 lg:mt-3 space-y-1 lg:space-y-1.5">
                              {[
                                { met: data.password.length >= 8, label: 'យ៉ាងតិច 8 តួអក្សរ' },
                                { met: /[a-z]/.test(data.password), label: 'យ៉ាងតិច 1 អក្សរតូច' },
                                { met: /[A-Z]/.test(data.password), label: 'យ៉ាងតិច 1 អក្សរធំ' },
                                { met: /[\d!@#$%^&*(),.?":{}|<>\s]/.test(data.password), label: 'យ៉ាងតិច 1 តួលេខ ឬសញ្ញា' },
                              ].map(({ met, label }) => (
                                <div key={label} className="flex items-center gap-1.5 lg:gap-2">
                                  <div className={`w-1.5 h-1.5 lg:w-2 lg:h-2 rounded-full ${met ? 'bg-emerald-500' : 'bg-gray-300'}`} />
                                  <span className={`text-[10px] sm:text-xs lg:text-sm ${met ? 'text-emerald-600' : 'text-gray-500'}`}>{label}</span>
                                </div>
                              ))}
                            </div>
                            {errors.password && <p className="text-red-600 text-[10px] sm:text-xs mt-0.5 lg:mt-1">{errors.password}</p>}
                          </div>
                ...
                {/* ---- FIX 5: Navigation buttons INSIDE form ---- */}
                <div className="flex-shrink-0 pt-3 sm:pt-4 lg:pt-5 mt-3 sm:mt-4 lg:mt-5 border-t flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
                    ...
                    {/* ---- FIX 6: last step uses type="submit" directly; other steps are type="button" ---- */}
                    {!isOnLastStep ? (
                      <button
                        type="button"
                        onClick={handleNext}
          ...
          <div className="hidden lg:flex lg:w-1/3 bg-gradient-to-b from-emerald-600 to-green-700 text-white p-6 lg:p-8 flex-col justify-between overflow-y-auto">
            <div>
              ...
              <div className="space-y-4 lg:space-y-5">
                ...
                <div className="flex items-start gap-2.5 lg:gap-3">
                  <div className="bg-white/20 p-2 rounded-lg flex-shrink-0">
                    <TruckIcon className="w-4 h-4 lg:w-5 lg:h-5" />
                  </div>
                  <div>
                ...
                <div className="flex items-start gap-2.5 lg:gap-3">
                  <div className="bg-white/20 p-2 rounded-lg flex-shrink-0">
                    <ShieldIcon className="w-4 h-4 lg:w-5 lg:h-5" />
                  </div>
                  <div>
      ...
      <style>{`
        ...
        .scrollbar-thin::-webkit-scrollbar-track { background: #f9fafb; border-radius: 10px; }
...
// Inline SVG icons (renamed to avoid clash)
