...
import {
    ...
    FileText, AlertCircle, Save, Users2, ChevronRight, ChevronLeft,
} from 'lucide-react';
...
export default function EditSeller({ seller }: PageProps<{ seller: SellerUser }>) {
    ...
    // ── Phone availability check (excluding current user) ────────────────────
    const checkPhoneAvailability = async (phone: string): Promise<boolean | null> => {
        try {
            const resp = await axios.get('/check-phone', { params: { phone, except_id: seller.user_id } });
    ...
    const submit: FormEventHandler = (e) => {
        ...
        if (phoneStatus === 'taken') {
            toast.error('លេខទូរស័ព្ទនេះបានប្រើរួចហើយ។ សូមប្រើលេខផ្សេង។');
            return;
        }
        if (data.password && !isPasswordComplex) {
                    ...
                    <div className="mb-5 flex flex-shrink-0 items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-2">
                                <span className="text-base font-medium text-blue-700">ID: #{seller.user_id}</span>
                            </div>
                            ...
                            {activeTab === 0 && (
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                                        ...
                                        {/* Email — read-only (same as users/edit) */}
                                        <div>
                                            ...
                                            <div className="relative">
                                                ...
                                                <input type="email" value={data.email} readOnly
                                                    className={`${inputCls('email')} cursor-not-allowed bg-gray-100`} />
                                            </div>
                                            <Err f="email" />
                                        </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
                                        <div>
                                            ...
                                            <Err f="password" />
                                            {/* Password strength checklist — only when typing */}
                                            {data.password && (
